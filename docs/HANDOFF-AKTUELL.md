...
**🔴 OFFEN / als Nächstes:** (a) **Umbenennen auf der Leinwand** bleibt noch nicht (hängt am Preis-im-Titel). (b) **Mannschaft/Maschine pro Knoten** fehlt im Modell (Auftrag↔Ressource-Beziehung; heute nur baustellenweit). (c) „Pausiert" (onHold) hat kein Canvas-Gegenstück (braucht Frontend-Neubau des Bundles). (d) Verbinden-Dialog ist eine Liste — später evtl. das Ziehen DIREKT auf der Leinwand (braucht die React-Quelle des Bundles). Bestandsaufnahme des ganzen Canvas-Stapels (DA vs. FEHLT) steckt im Session-Verlauf.
